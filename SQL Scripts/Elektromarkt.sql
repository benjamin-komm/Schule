...
create table BELIEFERT(Haendler integer not null,Filiale integer not null,PRIMARY KEY(Haendler, Filiale),FOREIGN KEY (Haendler) REFERENCES GROSSHAENDLER(Nr) ON DELETE CASCADE,FOREIGN KEY (Filiale) REFERENCES FILIALE(Nr) ON DELETE CASCADE);
...
ALTER TABLE FILIALE ADD CONSTRAINT `FK_filiale_1` FOREIGN KEY `FK_filiale_1` (`Leiter`) REFERENCES MITARBEITER (`Nr`);